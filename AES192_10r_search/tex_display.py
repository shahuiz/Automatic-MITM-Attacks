...
#### Register variables  
    ...
    mc_inv_cost_fwd = np.ndarray(shape=(total_round+1, NCOL), dtype=int)
    mc_inv_cost_bwd = np.ndarray(shape=(total_round+1, NCOL), dtype=int)
    ...
    for r in range(total_round+1):
        for j in COL:
            mc_inv_cost_fwd[r,j] = Sol["MC_INV_Cost_fwd[%d,%d]" %(r,j)]
            mc_inv_cost_bwd[r,j] = Sol["MC_INV_Cost_bwd[%d,%d]" %(r,j)]
    ...
    for r in range(total_round - 1):
        ...
        if xor_rhs_cost_fwd[r,:].any() or xor_rhs_cost_bwd[r,:].any():
            f.write('\\path (%f,%f) node {\\tiny Add KR};\n'  %(NCOL//2, -NROW//4))
        ...
        # Add inv MC cost
        f.write('\\begin{scope}[xshift = %f cm, yshift = %f cm]\n\n'   %(slot*xtab+NCOL, -r*ytab-2*NROW))
        if mc_inv_cost_fwd[r,:].any() or mc_inv_cost_bwd[r,:].any():
            f.write('\\path (%f,%f) node {\\tiny MC};\n'  %(NCOL//2, -NROW//4))
            f.write('\\path (%f,%f) node {\\scriptsize$(-%d ~\\DoFF, -%d ~\\DoFB)$};\n'  %(NCOL//2, -NROW//2, np.sum(mc_inv_cost_fwd[r,:]), np.sum(mc_inv_cost_bwd[r,:])))
        f.write('\n'+'\\end{scope}'+'\n')
        f.write('\n\n')
    ...
    for r in range(Nr):
        ...
        if r == key_start_round:
            f.write('\\path (' + str(NCOL//2) + ',' + str(-0.8) + ') node {\\scriptsize$(+' + str(ini_df_key_b) + '~\\DoFF,~+' + str(ini_df_key_r) + '~\\DoFB)$};'+'\n')
            f.write('\\path (' + str(Nk) + ',' + str(0.8) + ') node {\\scriptsize$\\StKSA$};'+'\n')
